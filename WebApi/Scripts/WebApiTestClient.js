...
(function () {
    ...
    function TestClientViewModel(data) {
        ...
        var headers = self.RequestHeaders;
        addOrReplaceHeader(headers, "opcode", "");
        var headers = self.RequestHeaders;
        addOrReplaceHeader(headers, "orgid", "");
        self.RequestBody.subscribe(function () {
            var headers = self.RequestHeaders;
            addOrReplaceHeader(headers, "access_token", $("#accessToken").val());
        });
        self.RequestHeaders.splice(0, 0, { name: "access_token", value: $("#accessToken").val() });
...
function SendRequest(httpMethod, url, requestHeaders, requestBody, handleResponse) {
    ...
    try {
        ...
        httpRequest.send($("#sampleArea").val());
    } catch (e) {
        alert(e);
        return false;
    }
